...
{
    // sibling of SpawnJuggernogOnGameStartedPatch. fires once at raid start
    // when ZombiesMode is on, resolves the current map id, looks up the
    // per-map Staminup config, and (if enabled) instantiates the perk-machine
    // prefab at the configured pose.
    public class SpawnStaminupOnGameStartedPatch : ModulePatch
    {
        ...
        private static async Task SpawnAsync(GameWorld gw)
        {
            try
            {
                ...
                {
                    Plugin.LogSource.LogInfo("Staminup: Zombies Mode is off; skipping spawn.");
                    return;
                }
                ...
                {
                    Plugin.LogSource.LogWarning("Staminup: map id unknown, skipping spawn.");
                    return;
                }

                StaminupMapSpawnConfig.Entry entry = StaminupMapSpawnConfig.GetForMap(mapId);
                if (entry == null)
                {
                    Plugin.LogSource.LogInfo($"Staminup: no config section for map '{mapId}', skipping.");
                    return;
                }
                if (!entry.TryGetTransform(out Vector3 pos, out Quaternion rot))
                {
                    Plugin.LogSource.LogInfo($"Staminup: map '{mapId}' disabled or coords unparseable, skipping.");
                    return;
                }

                GameObject prefab = await PerkMachineBundleLoader.Staminup.EnsureLoaded();
                if (prefab == null)
                {
                    Plugin.LogSource.LogWarning("Staminup: prefab not loaded from staminup_machine.bundle; skipping spawn.");
                    return;
                }
                ...
                StaminupInstance ctrl = instance.AddComponent<StaminupInstance>();
                ctrl.Initialize(gw.MainPlayer, entry, interactionTrigger);

                StaminupMachine machine = instance.AddComponent<StaminupMachine>();
                machine.Configure($"staminup:{mapId}:{instance.GetInstanceID()}");

                Plugin.LogSource.LogInfo($"Staminup: spawned on '{mapId}' at pos={pos} eul={rot.eulerAngles}");
            }
            catch (System.Exception e)
            {
                Plugin.LogSource.LogError($"Staminup spawn failed: {e}");
            }
        }
        ...
        private static BoxCollider EnsureInteractionTrigger(Transform root)
        {
            ...
            {
                Plugin.LogSource?.LogWarning("[Staminup] no MeshRenderers under prefab root; trigger left at default 1x1x1 cube.");
                return box;
            }
