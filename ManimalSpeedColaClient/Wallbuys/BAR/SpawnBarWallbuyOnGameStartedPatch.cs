...
{
    // sibling of SpawnUmpWallbuyOnGameStartedPatch / SpawnStgWallbuyOnGameStartedPatch.
    // spawns the BAR wallbuy at raid start; BarWallbuyActionPatch injects
    // the buy behavior. preloads every viewmodel bundle the BAR build will
    // need so Player.Proceed can swap hands cleanly without a "bundle not
    // loaded" NRE on first buy.
    public class SpawnBarWallbuyOnGameStartedPatch : ModulePatch
    {
        ...
        private static async Task SpawnAsync(GameWorld gw)
        {
            try
            {
                ...
                {
                    Plugin.LogSource.LogInfo("BarWallbuy: Zombies Mode is off; skipping spawn.");
                    return;
                }
                ...
                {
                    Plugin.LogSource.LogWarning("BarWallbuy: map id unknown, skipping spawn.");
                    return;
                }

                BarWallbuyMapSpawnConfig.Entry entry = BarWallbuyMapSpawnConfig.GetForMap(mapId);
                if (entry == null)
                {
                    Plugin.LogSource.LogInfo($"BarWallbuy: no config section for map '{mapId}', skipping.");
                    return;
                }
                if (!entry.TryGetTransform(out Vector3 pos, out Quaternion rot))
                {
                    Plugin.LogSource.LogInfo($"BarWallbuy: map '{mapId}' disabled or coords unparseable, skipping.");
                    return;
                }

                GameObject prefab = await WallbuyBundleLoader.Bar.EnsureLoaded();
                if (prefab == null)
                {
                    Plugin.LogSource.LogWarning("BarWallbuy: prefab not loaded from barwallbuy bundle; skipping spawn.");
                    return;
                }
                ...
                BarWallbuy wallbuy = instance.AddComponent<BarWallbuy>();
                wallbuy.Configure($"barwallbuy:{mapId}:{instance.GetInstanceID()}", entry, interactionTrigger);

                Plugin.LogSource.LogInfo($"BarWallbuy: spawned on '{mapId}' at pos={pos} eul={rot.eulerAngles}");

                _ = PreloadBundlesAsync(BarWallbuyActionPatch.RequiredBundleTpls);
            }
            catch (Exception e)
            {
                Plugin.LogSource.LogError($"BarWallbuy spawn failed: {e}");
            }
        }
        ...
        private static BoxCollider EnsureInteractionTrigger(Transform root)
        {
            ...
            {
                Plugin.LogSource?.LogWarning("[BarWallbuy] no MeshRenderers under prefab root; trigger left at default 1x1x1 cube.");
                return box;
            }
        ...
        private static async Task PreloadBundlesAsync(string[] tpls)
        {
            try
            {
                ...
                Plugin.LogSource?.LogInfo($"[BarWallbuy] preloaded {keys.Count} bundle(s) for {tpls.Length} tpl(s).");
            }
            catch (Exception ex)
            {
                Plugin.LogSource?.LogError($"[BarWallbuy] PreloadBundlesAsync failed: {ex.GetType().Name}: {ex.Message}");
            }
        }
    }
}
