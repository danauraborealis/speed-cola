...
{
    // sibling of BarWallbuyActionPatch for the SKS. tpls + slot layout come
    // from D:\SPTDev\SPT\user\mods\WTT-PresetConverterPlusServer\Presets\GlobalPresets\zombiesSKS.json
    //
    // tree:
    //   root (SKS)
    //     mod_stock
    ...
    //       mod_mount_000
    //         mod_gas_block
    //           mod_mount_000 (gas-block top mount)
    //         mod_sight_rear
    //     mod_magazine            -> 10rd box mag, loaded with ammo
    //     mod_reciever
    //       mod_scope
    //     patron_in_weapon        -> 1 chambered round
    //
    // 600 TC flat for first buy. re-buy is half price (300 TC) and just
    // refills chambers + tops off mags + repairs durability.
    internal sealed class SksWallbuyActionPatch : ModulePatch
    {
        public const int TarCoinPrice = 600;

        public const string SksRootTpl = "574d967124597745970e7c94"; // SKS
        public const string SksMagTpl  = "587df583245977373c4f1129"; // 10rd box mag
        public const string SksAmmoTpl = "64b7af734b75259c590fa895"; // 7.62x39 round
        ...
        public static readonly string[] RequiredBundleTpls =
        {
            ...
            SksMagTpl,
            SksAmmoTpl,
            // mod bundles - all the attached mods so the held-weapon prefab
            // chain resolves at SwitchToWeapon time without a cold-pool stall.
            "653ecef836fae5a82f02b869", // mod_stock
            "634f02331f9f536910079b51", // mod_barrel
            "6415c694da439c6a97048b56", // mod_reciever
            "634f04d82e5def262d0b30c6", // barrel mod_mount_000
            "634f02d7517ccc8a960fc744", // mod_gas_block
            "574db213245977459a2f3f5d", // mod_sight_rear
            "653ecc425a1690d9d90491e4", // gas-block mod_mount_000
            "570fd79bd2720bc7458b4583", // mod_scope
        };
        ...
        private static readonly PresetEntry[] SksTree =
        {
            new PresetEntry("root",       SksRootTpl,                       null,        null),
            new PresetEntry("stock",      "653ecef836fae5a82f02b869",       "root",      "mod_stock"),
            new PresetEntry("barrel",     "634f02331f9f536910079b51",       "root",      "mod_barrel"),
            new PresetEntry("magazine",   SksMagTpl,                        "root",      "mod_magazine"),
            new PresetEntry("reciever",   "6415c694da439c6a97048b56",       "root",      "mod_reciever"),
            new PresetEntry("chamber",    SksAmmoTpl,                       "root",      "patron_in_weapon"),
            new PresetEntry("barrel_mt",  "634f04d82e5def262d0b30c6",       "barrel",    "mod_mount_000"),
            new PresetEntry("gas_block",  "634f02d7517ccc8a960fc744",       "barrel_mt", "mod_gas_block"),
            new PresetEntry("sight_rear", "574db213245977459a2f3f5d",       "barrel_mt", "mod_sight_rear"),
            new PresetEntry("gas_mt",     "653ecc425a1690d9d90491e4",       "gas_block", "mod_mount_000"),
            new PresetEntry("scope",      "570fd79bd2720bc7458b4583",       "reciever",  "mod_scope"),
        };
        ...
        {
            SksWallbuy wallbuy = interactive as SksWallbuy;
            if (wallbuy == null) return true;

            try { __result = BuildActions(owner, wallbuy); }
            catch (Exception ex)
            {
                Plugin.LogSource?.LogError($"[SksWallbuy] action build failed: {ex.Message}");
                __result = new ActionsReturnClass { Actions = new List<ActionsTypesClass>() };
            }
            return false;
        }

        private static ActionsReturnClass BuildActions(GamePlayerOwner owner, SksWallbuy wallbuy)
        {
            ...
            bool isRefill = inv != null && FindFirstByTpl(inv.Inventory.Equipment, SksRootTpl) != null;
        ...
        private static void OnBuy(SksWallbuy wallbuy, Player player, int price)
        {
            try
            {
                if (!TarCoinWallet.TrySpend(player, price))
                {
                    Plugin.LogSource?.LogInfo("[SksWallbuy] not enough TarCoins; buy aborted.");
                    return;
                }
            ...
            catch (Exception ex) { Plugin.LogSource?.LogError($"[SksWallbuy] buy failed: {ex.Message}"); }
        }
        ...
        private static async Task DispenseAsync()
        {
            ...
            {
                Plugin.LogSource?.LogWarning("[SksWallbuy] ItemFactory or InventoryController missing.");
                return;
            }

            Item existing = FindFirstByTpl(inv.Inventory.Equipment, SksRootTpl);
            if (existing != null)
            {
            ...
            Item weapon = BuildPresetTree(inv, factory, SksTree);
            if (weapon == null) return;
            weapon.SpawnedInSession = true;

            LoadMagazineInSlot(weapon, "mod_magazine", SksAmmoTpl, inv, factory);

            EquipmentSlot targetSlot = ResolveTargetSlot(gw.MainPlayer, inv);
            Plugin.LogSource?.LogInfo($"[SksWallbuy] target slot: {targetSlot}");

            Slot equipSlot = inv.Inventory.Equipment.GetSlot(targetSlot);
            if (equipSlot.ContainedItem != null)
            {
                Plugin.LogSource?.LogInfo($"[SksWallbuy] {targetSlot} occupied by {equipSlot.ContainedItem.TemplateId}; replacing.");
                DiscardItem(equipSlot.ContainedItem, inv);
                WallbuyAmmoTracker.DiscardForSlot(targetSlot, inv);
            }

            List<Item> givenMags = await AddLoadedMagStacks(inv, factory, SksMagTpl, SksAmmoTpl, MagCount);
            ...
            Plugin.LogSource?.LogInfo($"[SksWallbuy] SKS equipped in {targetSlot}.");
        ...
        public static System.Threading.Tasks.Task RefillForMaxAmmoPickup(Player player)
        {
            try
            {
                ...
                Item weapon = FindFirstByTpl(inv.Inventory.Equipment, SksRootTpl);
                if (weapon == null)
                {
                    Plugin.LogSource?.LogInfo("[MaxAmmo] SKS refill skipped: player doesn't own an SKS.");
                    return System.Threading.Tasks.Task.CompletedTask;
                }
                ItemFactoryClass factory = Singleton<ItemFactoryClass>.Instance;
                if (factory == null) return System.Threading.Tasks.Task.CompletedTask;
                Plugin.LogSource?.LogInfo("[MaxAmmo] SKS refill running...");
                return RestockExisting(weapon, inv, factory);
            }
            catch (Exception ex)
            {
                Plugin.LogSource?.LogWarning($"[MaxAmmo] SKS RefillForMaxAmmoPickup threw: {ex.Message}");
                return System.Threading.Tasks.Task.CompletedTask;
            }
        }
        ...
        private static async Task RestockExisting(Item weapon, InventoryController inv, ItemFactoryClass factory)
        {
            ...
            {
                RefillChambers(w, SksAmmoTpl, inv, factory);
            ...
            if (inWeaponMag != null) RefillMagazineToCapacity(inWeaponMag, SksAmmoTpl, inv, factory);

            int spareCount = 0;
            foreach (Item it in inv.Inventory.Equipment.GetAllItemsFromCollection())
            {
                if (it == null) continue;
                if (it.TemplateId != SksMagTpl) continue;
                if (it == inWeaponMag) continue;
                RefillMagazineToCapacity(it, SksAmmoTpl, inv, factory);
                spareCount++;
            }
            ...
            if (needed > 0)
            {
                List<Item> givenMags = await AddLoadedMagStacks(inv, factory, SksMagTpl, SksAmmoTpl, needed);
            ...
            Plugin.LogSource?.LogInfo($"[SksWallbuy] restock: chambers refilled, spare mags={spareCount}, dispensed {Math.Max(0, needed)} fresh.");
        }
        ...
        private static Item BuildPresetTree(InventoryController inv, ItemFactoryClass factory, PresetEntry[] preset)
        {
            ...
            while (pending.Count > 0 && safety-- > 0)
            {
                ...
                for (int i = pending.Count - 1; i >= 0; i--)
                {
                    ...
                    if (entry.ParentPresetId == null)
                    {
                        ...
                        if (root == null)
                        {
                            Plugin.LogSource?.LogError($"[SksWallbuy] CreateItem({entry.Tpl}) returned null for root.");
                            return null;
                        }
                    ...
                    if (slot == null)
                    {
                        Plugin.LogSource?.LogWarning($"[SksWallbuy] slot '{entry.SlotId}' missing on {parent.TemplateId}; skipping mod {entry.Tpl}.");
                        pending.RemoveAt(i);
                        progress = true;
                        continue;
                    }
                    ...
                    if (mod == null)
                    {
                        Plugin.LogSource?.LogWarning($"[SksWallbuy] CreateItem({entry.Tpl}) returned null; skipping.");
                        pending.RemoveAt(i);
                        progress = true;
                        continue;
                    }
            ...
            if (pending.Count > 0)
                Plugin.LogSource?.LogWarning($"[SksWallbuy] {pending.Count} preset entries could not be attached (parents never resolved).");
        ...
        private static Task<List<Item>> AddLoadedMagStacks(InventoryController inv, ItemFactoryClass factory, string magTpl, string ammoTpl, int count)
        {
            ...
            Plugin.LogSource?.LogInfo($"[SksWallbuy] dispensed {added.Count}/{count} loaded mag(s) to inventory.");
        ...
        private static EquipmentSlot ResolveTargetSlot(Player player, InventoryController inv)
        {
            ...
            Plugin.LogSource?.LogInfo("[SksWallbuy] both primaries full and player not holding a primary; defaulting to FirstPrimaryWeapon.");
        ...
        private static void DiscardItem(Item item, InventoryController inv)
        {
            try
            {
                ...
                if (result.Failed)
                    Plugin.LogSource?.LogWarning($"[SksWallbuy] Discard failed for {item?.TemplateId}: {result.Error}");
            }
            catch (Exception ex)
            {
                Plugin.LogSource?.LogError($"[SksWallbuy] Discard threw for {item?.TemplateId}: {ex.Message}");
            }
        }
        ...
        private static void SwitchToWeapon(Player player, Item weapon)
        {
            try
            {
                ...
                if (w == null)
                {
                    Plugin.LogSource?.LogWarning("[SksWallbuy] cannot switch to weapon: cast to Weapon failed.");
                    return;
                }
                player.Proceed(w, new Callback<IFirearmHandsController>(result =>
                {
                    if (result.Failed)
                        Plugin.LogSource?.LogWarning($"[SksWallbuy] auto-switch failed: {result.Error}");
                }), scheduled: true);
            }
            catch (Exception ex)
            {
                Plugin.LogSource?.LogError($"[SksWallbuy] SwitchToWeapon threw: {ex.Message}");
            }
        }
    }
}
