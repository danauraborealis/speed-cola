...
{
    // sibling of SpawnUmpWallbuyOnGameStartedPatch. spawns the STG-44 wallbuy
    // at raid start; StgWallbuyActionPatch injects the buy behavior. also
    // preloads every viewmodel bundle the STG build will need so Player.Proceed
    // can swap hands cleanly without a "bundle not loaded" NRE on first buy.
    public class SpawnStgWallbuyOnGameStartedPatch : ModulePatch
    {
        ...
        private static async Task SpawnAsync(GameWorld gw)
        {
            try
            {
                ...
                {
                    Plugin.LogSource.LogInfo("StgWallbuy: Zombies Mode is off; skipping spawn.");
                    return;
                }
                ...
                {
                    Plugin.LogSource.LogWarning("StgWallbuy: map id unknown, skipping spawn.");
                    return;
                }

                StgWallbuyMapSpawnConfig.Entry entry = StgWallbuyMapSpawnConfig.GetForMap(mapId);
                if (entry == null)
                {
                    Plugin.LogSource.LogInfo($"StgWallbuy: no config section for map '{mapId}', skipping.");
                    return;
                }
                if (!entry.TryGetTransform(out Vector3 pos, out Quaternion rot))
                {
                    Plugin.LogSource.LogInfo($"StgWallbuy: map '{mapId}' disabled or coords unparseable, skipping.");
                    return;
                }

                GameObject prefab = await WallbuyBundleLoader.Stg.EnsureLoaded();
                if (prefab == null)
                {
                    Plugin.LogSource.LogWarning("StgWallbuy: prefab not loaded from stgwallbuy bundle; skipping spawn.");
                    return;
                }
                ...
                StgWallbuy wallbuy = instance.AddComponent<StgWallbuy>();
                wallbuy.Configure($"stgwallbuy:{mapId}:{instance.GetInstanceID()}", entry, interactionTrigger);

                Plugin.LogSource.LogInfo($"StgWallbuy: spawned on '{mapId}' at pos={pos} eul={rot.eulerAngles}");

                _ = PreloadBundlesAsync(StgWallbuyActionPatch.RequiredBundleTpls);
            }
            catch (Exception e)
            {
                Plugin.LogSource.LogError($"StgWallbuy spawn failed: {e}");
            }
        }
        ...
        private static BoxCollider EnsureInteractionTrigger(Transform root)
        {
            ...
            {
                Plugin.LogSource?.LogWarning("[StgWallbuy] no MeshRenderers under prefab root; trigger left at default 1x1x1 cube.");
                return box;
            }
        ...
        private static async Task PreloadBundlesAsync(string[] tpls)
        {
            try
            {
                ...
                Plugin.LogSource?.LogInfo($"[StgWallbuy] preloaded {keys.Count} bundle(s) for {tpls.Length} tpl(s).");
            }
            catch (Exception ex)
            {
                Plugin.LogSource?.LogError($"[StgWallbuy] PreloadBundlesAsync failed: {ex.GetType().Name}: {ex.Message}");
            }
        }
    }
}
